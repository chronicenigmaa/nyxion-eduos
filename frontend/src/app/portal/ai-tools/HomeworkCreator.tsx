...
import { ClipboardList, Sparkles, Loader2, Copy, Check } from "lucide-react";

export default function HomeworkCreator({ onBack }: { onBack: () => void }) {
...
Mix short answer, fill-in-the-blank, and one application question. Include an answer key.`,
      });
  ...
  return (
    <div className="p-8 max-w-3xl">
      ...
      <div className="flex items-center gap-2 mb-6"><ClipboardList size={20} className="text-blue-600" /><h1 className="text-xl font-bold text-slate-900">Homework Creator</h1></div>
      <div className="bg-white border border-slate-200 rounded-2xl p-6 mb-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div><label className="block text-xs font-medium text-slate-600 mb-1">Subject *</label>
            <input value={subject} onChange={e => setSubject(e.target.value)} placeholder="e.g. English"
        ...
        <div><label className="block text-xs font-medium text-slate-600 mb-1">Topic (optional)</label>
          <input value={topic} onChange={e => setTopic(e.target.value)} placeholder="e.g. Tenses"
            className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" /></div>
        <div><label className="block text-xs font-medium text-slate-600 mb-1">Number of Questions</label>
      ...
      {result && (
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
          <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100 bg-slate-50">
            <span className="text-sm font-medium text-slate-700">Homework Worksheet</span>
