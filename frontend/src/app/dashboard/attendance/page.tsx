...
export default function AttendancePage() {
  ...
  const markAll = (status: string) => {
    const m = {...attendance};
    filteredRecords.forEach(r => { m[r.student_id] = status; });
    setAttendance(m);
  };
  ...
  const absentNames = records.filter(r => attendance[r.student_id] === "absent").map(r => r.student_name);
  const lateNames = records.filter(r => attendance[r.student_id] === "late").map(r => r.student_name);
  const attSummary = "Date: "+date+". Total: "+summary.total+", Present: "+summary.present+", Absent: "+summary.absent+", Late: "+summary.late+", Rate: "+rate+"%. Absent: "+(absentNames.join(", ")||"none")+". Late: "+(lateNames.join(", ")||"none");

  const aiOptions = [
    { label: "Daily Report", icon: TrendingUp, type: "attendance", prompt: "Write a concise daily attendance report for school management. "+attSummary },
    { label: "At-Risk Students", icon: AlertCircle, type: "attendance", prompt: "Identify students needing follow-up based on today's attendance. "+attSummary+". Who needs intervention?" },
    { label: "Improvement Tips", icon: Lightbulb, type: "attendance", prompt: "Give 5 actionable tips to improve student attendance. Context: "+attSummary },
    { label: "Parent Messages", icon: Users, type: "attendance", prompt: "Write short WhatsApp messages in English and Urdu for parents of absent students: "+absentNames.join(", ")+". Keep professional and brief." },
  ];

  const statusBtn = (cur: string, tgt: string) => {
    if (cur === tgt) return ({present:"bg-green-100 text-green-700 border border-green-200",absent:"bg-red-100 text-red-700 border border-red-200",late:"bg-amber-100 text-amber-700 border border-amber-200",leave:"bg-slate-100 text-slate-700 border border-slate-200"}[tgt]||"");
    return "bg-white text-slate-400 border border-slate-200 hover:border-slate-300";
  };
  ...
  return (
    <div className="p-8">
      ...
      <AIInsightsPanel options={aiOptions} title="Attendance AI Analysis" />
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <button onClick={()=>markAll("present")} className="px-3 py-2 rounded-xl bg-green-50 text-green-700 border border-green-200 text-sm hover:bg-green-100">All Present</button>
        <button onClick={()=>markAll("absent")} className="px-3 py-2 rounded-xl bg-red-50 text-red-700 border border-red-200 text-sm hover:bg-red-100">All Absent</button>
      </div>
                    ...
                    <div className="flex gap-2">
                      {["present","absent","late","leave"].map(s=>(
                        <button key={s} onClick={()=>setAttendance({...attendance,[r.student_id]:s})}
                          className={"px-3 py-1 rounded-lg text-xs font-medium capitalize transition-all "+statusBtn(attendance[r.student_id],s)}>{s}</button>
                      ))}
                    </div>
                  </td>
                </tr>
