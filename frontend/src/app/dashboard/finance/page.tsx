...
export default function FinancePage() {
  ...
  const pendingNames = fees.filter(f=>f.status==="pending").slice(0,8).map(f=>f.student_name);
  const finSummary = "Total fees: "+summary.total+", Paid: "+summary.paid+", Pending: "+summary.pending+", Overdue: "+summary.overdue+". Total: PKR "+summary.total_amount.toLocaleString()+", Collected: PKR "+summary.collected.toLocaleString()+", Rate: "+rate+"%. Pending students: "+(pendingNames.join(", ")||"none");

  const aiOptions = [
    { label: "Collection Report", icon: DollarSign, type: "finance", prompt: "Write a monthly fee collection report for school management. "+finSummary },
    { label: "Defaulter Analysis", icon: AlertCircle, type: "finance", prompt: "Analyze fee defaulters and identify patterns. "+finSummary+". Who are the high-risk defaulters?" },
    { label: "Revenue Forecast", icon: TrendingUp, type: "finance", prompt: "Forecast next month revenue based on: "+finSummary+". What improvement strategies do you recommend?" },
    { label: "Recovery Strategy", icon: Lightbulb, type: "finance", prompt: "Create a fee recovery strategy with WhatsApp message templates in English and Urdu for: "+finSummary },
  ];
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white rounded-2xl border border-green-200 p-5"><p className="text-slate-500 text-sm">Collected</p><p className="text-2xl font-bold text-green-600 mt-1">PKR {summary.collected.toLocaleString()}</p><p className="text-xs text-green-500 mt-1">{rate}% rate</p></div>
      ...
      <AIInsightsPanel options={aiOptions} title="Finance AI Analysis" />
