...
interface Teacher {
  ...
  subject: string; qualification: string; salary: string;
}
...
export default function TeachersPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try { await api.post("/api/v1/teachers/", form); toast.success("Teacher added!"); setShowForm(false); setForm({ full_name: "", email: "", phone: "", subject: "", qualification: "", salary: "" }); load(); }
  ...
  const getAIInsight = async () => {
    ...
    try {
      ...
      const { data } = await api.post("/api/v1/ai/analyze", { prompt: "Analyze teacher data: " + summary + ". Provide insights on staff coverage and recommendations.", type: "academic" });
      setAiInsight(data.response);
    } catch { toast.error("AI unavailable"); }
    finally { setAiLoading(false); }
  };

  const filtered = teachers.filter(t => t.full_name?.toLowerCase().includes(search.toLowerCase()) || t.subject?.toLowerCase().includes(search.toLowerCase()));
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-500">{teachers.length} staff members</p>
        </div>
        <div className="flex gap-2">
          <button onClick={getAIInsight} disabled={aiLoading} className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 text-sm font-medium transition-all">
            {aiLoading ? <Loader2 size={16} className="animate-spin" /> : <Brain size={16} className="text-blue-600" />} AI Insights
          </button>
      ...
      {aiInsight && (
        <div className="bg-blue-50 border border-blue-200 rounded-2xl p-6 mb-6">
          <div className="flex items-center gap-2 mb-3"><Brain size={16} className="text-blue-600" /><span className="text-blue-700 text-sm font-medium">AI Analysis</span><button onClick={() => setAiInsight("")} className="ml-auto text-slate-400 hover:text-slate-600 text-xs">dismiss</button></div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5"><p className="text-slate-500 text-sm">Subjects Covered</p><p className="text-3xl font-bold text-blue-600 mt-1">{[...new Set(teachers.map(t => t.subject).filter(Boolean))].length}</p></div>
        <div className="bg-white rounded-2xl border border-slate-200 p-5"><p className="text-slate-500 text-sm">Avg Salary</p><p className="text-3xl font-bold text-green-600 mt-1">{teachers.filter(t => t.salary).length > 0 ? Math.round(teachers.filter(t => t.salary).reduce((a, t) => a + parseInt(t.salary || "0"), 0) / teachers.filter(t => t.salary).length).toLocaleString() : "-"}</p></div>
      </div>
            ...
            {loading ? <tr><td colSpan={7} className="text-center text-slate-400 py-12">Loading...</td></tr>
              ...
              : filtered.map((t) => (
                <tr key={t.id} className="border-b border-slate-50 hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3"><button onClick={() => handleDelete(t.id)} className="text-slate-300 hover:text-red-500"><Trash2 size={15} /></button></td>
                </tr>
              ))}
          </tbody>
        </table>
