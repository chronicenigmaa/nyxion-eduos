  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data?.login_created && form.email) {
        const password = data?.temporary_password || form.temporary_password;
        toast.success(`Login created for ${form.email}. Temporary password: ${password}`);
      }
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <ExportButton title="Teachers Report" data={filtered} filename="teachers-report" />
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5"><p className="text-slate-500 text-sm">Avg Salary (PKR)</p><p className="text-3xl font-bold text-green-600 mt-1">{teachers.filter((t) => t.salary).length > 0 ? Math.round(teachers.filter((t) => t.salary).reduce((a, t) => a + parseInt(t.salary || "0", 10), 0) / teachers.filter((t) => t.salary).length).toLocaleString() : "-"}</p></div>
      </div>
      ...
      {showForm && (
        <div className="bg-white rounded-2xl border border-slate-200 p-6 mb-6">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <p className="col-span-full text-xs text-slate-400 -mt-1">If email is provided, teacher login is created automatically. Leave temporary password empty to auto-generate one.</p>
            ...
            {loading ? <tr><td colSpan={7} className="text-center text-slate-400 py-12">Loading...</td></tr>
              ...
              : filtered.map((t) => (
                <tr key={t.id} className="border-b border-slate-50 hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-green-600 text-sm font-medium">{t.salary ? `PKR ${parseInt(t.salary, 10).toLocaleString()}` : "-"}</td>
