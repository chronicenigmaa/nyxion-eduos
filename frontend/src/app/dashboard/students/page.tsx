...
interface Student {
  id: string; full_name: string; father_name: string;
  roll_number: string; class_name: string; section: string; phone: string;
}
...
export default function StudentsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try { await api.post("/api/v1/students/", form); toast.success("Student added!"); setShowForm(false); setForm({ full_name: "", father_name: "", roll_number: "", class_name: "", section: "", phone: "" }); load(); }
        ...
        <table className="w-full">
          ...
          <tbody>
            {loading ? <tr><td colSpan={7} className="text-center text-slate-400 py-12">Loading...</td></tr>
              ...
              : filtered.map((s) => (
                <tr key={s.id} className="border-b border-slate-50 hover:bg-slate-50 transition-all">
                  ...
                  <td className="px-4 py-3">
                    <button onClick={() => handleDelete(s.id)} className="text-slate-300 hover:text-red-500 transition-all"><Trash2 size={15} /></button>
                  </td>
                </tr>
              ))}
          </tbody>
