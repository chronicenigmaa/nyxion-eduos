...
export default function StudentsPage() {
  ...
  const [form, setForm] = useState({ full_name: "", father_name: "", roll_number: "", class_name: "", section: "", phone: "" });

  const load = async () => {
    try { const { data } = await api.get("/api/v1/students/"); setStudents(data); }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try { await api.post("/api/v1/students/", form); toast.success("Added!"); setShowForm(false); setForm({ full_name:"",father_name:"",roll_number:"",class_name:"",section:"",phone:"" }); load(); }
    catch { toast.error("Failed"); }
  };

  const handleDelete = async (id: string) => {
    if (!confirm("Remove this student?")) return;
    try { await api.delete("/api/v1/students/"+id); toast.success("Removed"); load(); }
    catch { toast.error("Failed"); }
  };

  const classes = [...new Set(students.map(s => s.class_name).filter(Boolean))];
  const summary = "Total: "+students.length+" students. Classes: "+classes.join(", ")+". Distribution: "+classes.map(c => "Class "+c+": "+students.filter(s=>s.class_name===c).length+" students").join(", ");
    ...
    <div className="p-8">
      ...
      {showForm && (
        <div className="bg-white rounded-2xl border border-slate-200 p-6 mb-6">
          <h2 className="text-slate-900 font-semibold mb-4">New Student</h2>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        <table className="w-full">
          <thead><tr className="border-b border-slate-100 bg-slate-50">{["Roll No","Name","Father Name","Class","Section","Phone",""].map(h=><th key={h} className="text-left text-xs text-slate-500 font-medium px-4 py-3">{h}</th>)}</tr></thead>
          <tbody>
            {loading ? <tr><td colSpan={7} className="text-center text-slate-400 py-12">Loading...</td></tr>
              : filtered.length === 0 ? <tr><td colSpan={7} className="text-center py-12"><GraduationCap size={32} className="mx-auto text-slate-300 mb-2"/><p className="text-slate-400 text-sm">No students yet</p></td></tr>
              : filtered.map(s => (
